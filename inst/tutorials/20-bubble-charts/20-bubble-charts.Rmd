...
logs <- read_csv("https://www.mattwaite.com/sportsdatafiles/footballlogs25.csv")
...
ypp <- winlosslogs |> 
  ...
  summarise(
    ...
    total_yards = sum(tot), 
    defensive_plays = sum(opponent_plays), 
    defensive_yards = sum(opponent_tot), 
    total_wins = sum(wins)) |> 
  mutate(
    offensiveYPP = total_yards/total_plays, 
    defensiveYPP = defensive_yards/defensive_plays)
...
To do this, I've got some work to do. First, I need to mutate the results of the games into 1s and 0s so I can add up the wins. 
...
ggplot() + 
  geom_point(data=ypp, aes(x=offensiveYPP, y=defensiveYPP))
```
...
ggplot() + 
  geom_point(
    data=ypp, 
    aes(x=offensiveYPP, y=defensiveYPP, size=total_wins)
    )
```
```{r bubble4-check}
grade_this_code()
...
ggplot() + 
  ...
  geom_point(
    ...
    aes(x=offensiveYPP, y=defensiveYPP, size=total_wins), 
    color="red") + 
  geom_vline(xintercept = 5.714648) + 
  geom_hline(yintercept = 5.514364)
```
```{r bubble8-check}
grade_this_code()
```
