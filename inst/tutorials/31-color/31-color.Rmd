...
We're going to build a simple bar chart and layer in come color as an example. Let's look at Big Ten teams in the 2025 NCAA Tournament. So let's explore that. 
...
And the data. We're going to do four things here: first, we'll load in the season stats for each team in college basketball in 2024-2025. Then we'll filter out Big Ten teams to get us a dataframe of 18 teams called `big`. We'll then filter B1G teams that made the tournament. Lastly, we'll make a dataframe of just Michigan State, who seem to always be the last Big Ten team in the tournament.  

```{r colors-load-data, message=FALSE, warning=FALSE}
stats <- read_csv("https://mattwaite.github.io/sportsdatafiles/stats25.csv")

teams <- c("Nebraska", "Iowa", "Northwestern", "Minnesota", "Wisconsin", "Illinois", "Indiana", "Purdue", "Michigan", "Michigan State", "Ohio State", "Penn State", "Rutgers", "Maryland", "Oregon", "Washington", "USC", "UCLA")

tournament <- c("Illinois", "Maryland", "Michigan", "Michigan State", "Oregon", "Purdue", "UCLA", "Wisconsin")
...
Let's start by making a simple bar chart of `srs`. It's just a simple rating of each team. We're going to re-order it and coord_flip it like we've done before. 
...
ggplot() + 
  geom_bar(data=big, aes(x=reorder(school, srs), weight=srs)) + 
  coord_flip()
```
```{r colors-bar-check}
grade_this_code()
...
ggplot() + 
  geom_bar(data=big, aes(x=reorder(school, srs), weight=srs), fill="lightgrey") + 
  coord_flip()
```
```{r colors-bar2-check}
grade_this_code()
...
ggplot() + 
  geom_bar(data=big, aes(x=reorder(school, srs), weight=srs), fill="____") + 
  geom_bar(data=dance, aes(x=reorder(school, srs), weight=srs), fill="____") +  
  geom_bar(data=____, aes(x=reorder(school, srs), weight=srs), fill="#18453B") +    
  coord_flip()
```
```{r colors-bar3-solution, exercise.reveal_solution = FALSE}
ggplot() + 
  geom_bar(data=big, aes(x=reorder(school, srs), weight=srs), fill="lightgrey") + 
  geom_bar(data=dance, aes(x=reorder(school, srs), weight=srs), fill="darkgrey") +  
  geom_bar(data=mi, aes(x=reorder(school, srs), weight=srs), fill="#18453B") +    
  coord_flip()
```
```{r colors-bar3-check}
grade_this_code()
