...
difflogs <- logs |> 
  arrange(game_date) |> 
  mutate(differential = team_score - opponent_team_score) |> 
...
bigdiff <- difflogs |> filter(team_short_display_name %in% bigteams)

nu <- bigdiff |> filter(team_short_display_name == "Nebraska")
ms <- bigdiff |> filter(team_short_display_name == "Michigan St")
mi <- bigdiff |> filter(team_short_display_name == "Michigan")
```
...
____ <- difflogs |> filter(team_short_display_name %in% ____)
```
...
ggplot() + 
  ____(data=____, aes(x=game_date, y=____, group=team_short_display_name))
```
```{r step3-solution, exercise.reveal_solution = FALSE}
ggplot() + 
  geom_step(data=bigdiff, aes(x=game_date, y=cum_diff, group=team_short_display_name))
```
```{r step3-check}
grade_this_code()
```
...
Let's try a different element of the aesthetic: color, but this time *inside the aesthetic*. Last time, we did the color outside. When you put it inside, you pass it a column name -- like team -- and ggplot will color each line based on what thing that is, and it will create a legend that labels each line that thing. 

```{r step4, exercise=TRUE, exercise.setup = "step-load-data", message=FALSE}
ggplot() + 
  ____(data=____, aes(x=game_date, y=____, group=team_short_display_name, color=____))
```
```{r step4-solution, exercise.reveal_solution = FALSE}
ggplot() + 
  geom_step(data=bigdiff, aes(x=game_date, y=cum_diff, group=team_short_display_name, color=team_short_display_name))
```
```{r step4-check}
grade_this_code()
```
...
How do we sort this out? Layering and annotations. We've used layering before. Annotations, while new, will make sense in a minute. First, let's pick three teams to highlight -- Nebraska, Michigan and Michigan St (which in this data doesn't have the period after St). 

```{r step5, exercise=TRUE, exercise.setup = "step-load-data", message=FALSE}
nu <- bigdiff |> filter(team_short_display_name == "____")
ms <- bigdiff |> filter(team_short_display_name == "____")
mi <- bigdiff |> filter(team_short_display_name == "____")
```
...
ggplot() + 
  geom_step(data=bigdiff, aes(x=game_date, y=____, group=team_short_display_name), color="lightgrey") +
  geom_step(data=____, aes(x=game_date, y=cum_diff, group=team_short_display_name), color="red") + 
  geom_step(data=____, aes(x=game_date, y=cum_diff, group=team_short_display_name), color="darkgreen") +
  geom_step(data=____, aes(x=game_date, y=cum_diff, group=team_short_display_name), color="darkblue") +
  annotate("text", x=(as.Date("2025-12-15")), y=150, label="Nebraska") +
  annotate("text", x=(as.Date("2025-12-15")), y=210, label="Michigan St") +
...
ggplot() + 
  geom_step(data=bigdiff, aes(x=game_date, y=cum_diff, group=team_short_display_name), color="lightgrey") +
  geom_step(data=nu, aes(x=game_date, y=cum_diff, group=team_short_display_name), color="red") + 
  geom_step(data=ms, aes(x=game_date, y=cum_diff, group=team_short_display_name), color="darkgreen") +
  geom_step(data=mi, aes(x=game_date, y=cum_diff, group=team_short_display_name), color="darkblue") +
  annotate("text", x=(as.Date("2025-12-15")), y=150, label="Nebraska") +
  annotate("text", x=(as.Date("2025-12-15")), y=210, label="Michigan St") +
  annotate("text", x=(as.Date("2025-12-15")), y=290, label="Michigan") +
  labs(
