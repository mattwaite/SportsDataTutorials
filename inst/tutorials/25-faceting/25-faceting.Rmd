...
We'll do something simple like a line chart of `avg` -- the yards per play on offense -- and we're going to group on `team`.
...
ggplot() + 
  geom_line(data=big10, aes(x=date, y=avg, group=team))
```
```{r facet2-check}
grade_this_code()
```
...
ggplot() + 
  geom_hline(yintercept=5.871749, color="blue") + 
  geom_line(data=big10, aes(x=date, y=avg, group=team)) + 
  facet_wrap(~team)
```
```{r facet5-check}
