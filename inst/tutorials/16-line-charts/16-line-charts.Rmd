...
Let's look at how we can answer this question: How did last season's Nebraska's basketball season progress?

We'll use the game box score (the game log) of every game in college basketball. 
...
logs <- read_csv("https://www.mattwaite.com/sportsdatafiles/ncaa_mens_basketball_game_box_2026.csv")

nu <- logs |> filter(team_short_display_name == "Nebraska")
mi <- logs |> filter(team_short_display_name == "Michigan")

bigteams <- c("Nebraska", "Iowa", "Minnesota", "Illinois", "Northwestern", "Wisconsin", "Michigan", "Michigan", "Michigan St", "Ohio State", "Indiana", "Purdue", "Penn State", "Rutgers", "Maryland", "UCLA", "USC", "Washington", "Oregon")

big <- logs |> filter(team_short_display_name %in% bigteams)

average <- logs |> 
  group_by(game_date) |> 
  summarise(
    mean_shooting=mean(field_goal_pct)
    )
```
...
This data has every game from every team in it, so we need to use filtering to limit it, because we just want to look at Nebraska. If you don't remember, flip back to lesson 5. Let's make a new dataframe called nu for Nebraska's games. In this data, there are multiple team names. We're going to use the `team_short_display_name` because it keeps the names simple. If you look at that column in your head results above, I think you can figure out how to get Nebraska's games from that.  

```{r nu, exercise=TRUE, exercise.setup = "line-load-data", message=FALSE}
____ <- logs |> filter(team_short_display_name == "____")
```
...
Line charts, unlike bar charts, do have a y-axis. So in our ggplot step, we have to define what our x and y axes are. In this case, the x axis is our `game_date` -- *the most common x axis in line charts is going to be a date of some variety* -- and y in this case is up to us. How well a team shoots the ball has a lot to do with how well a team does in a season, so let's chart that using the field goal percentage column. 
...
ggplot() + 
  geom_line(data=nu, aes(x=game_date, y=field_goal_pct))
```
```{r line1-check}
grade_this_code()
```
...
Using a non-zero axis makes a line chart look more dramatic than it is. To make the axis what you want, you can use `scale_x_continuous` or `scale_y_continuous` and pass in a list with the bottom and top value you want. The bottom value is zero. The upper value is up to you. Let's try 70 percent.

```{r line2, exercise=TRUE, exercise.setup = "line-load-data", message=FALSE}
ggplot() + 
  geom_line(data=____, aes(x=____, y=field_goal_pct)) +
  scale_y_continuous(limits = c(0, ____))
```
...
With datasets, we want to invite comparison. So let's answer the question visually. Let's put two lines on the same chart. How does Nebraska compare to Michigan, for example? 
...
Let's add eventual national champions Michigan to this mix. 

```{r line3, exercise=TRUE, exercise.setup = "line-load-data", message=FALSE}
mi <- logs |> filter(team_short_display_name == "____")

ggplot() + 
  geom_line(data=____, aes(x=game_date, y=field_goal_pct), color="red") + 
  geom_line(data=____, aes(x=game_date, y=field_goal_pct), color="darkblue") +
  scale_y_continuous(limits = c(0, 70))
```
...
ggplot() + 
  geom_line(data=nu, aes(x=game_date, y=field_goal_pct), color="red") + 
  geom_line(data=mi, aes(x=game_date, y=field_goal_pct), color="darkblue") +
  scale_y_continuous(limits = c(0, 70))
```
```{r line3-check}
grade_this_code()
...
So visually speaking, what is the difference between Nebraska and Michigan's season? Does it tell a story?
...
NOTE: When you have more than one thing in a dataframe -- teams, players, whatever -- and you're making a line chart out of it, you need to add the `group` to the aesthetic. In this case, the group is the team name column we've been using.
...
ggplot() + 
  geom_line(data=____, aes(x=game_date, y=field_goal_pct, group=team_short_display_name), color="____") +
  geom_line(data=____, aes(x=game_date, y=field_goal_pct), color="red") + 
  geom_line(data=____, aes(x=game_date, y=field_goal_pct), color="darkblue") +
  scale_y_continuous(limits = c(0, 70))
```
...
ggplot() + 
  geom_line(data=big, aes(x=game_date, y=field_goal_pct, group=team_short_display_name), color="grey") +
  geom_line(data=nu, aes(x=game_date, y=field_goal_pct), color="red") + 
  geom_line(data=mi, aes(x=game_date, y=field_goal_pct), color="darkblue") +
  scale_y_continuous(limits = c(0, 70))
```
...
What do we see here? How has Nebraska and Michigan's season evolved against all the rest of the teams in the Big Ten?
...
average <- logs |> group_by(game_date) |> summarise(mean_shooting=mean(field_goal_pct))
```
```{r average-solution}
average <- logs |> group_by(game_date) |> summarise(mean_shooting=mean(field_goal_pct))
```
...
ggplot() + 
  geom_line(data=big, aes(x=game_date, y=team_fgpct, group=team_short_display_name), color="grey") +
  geom_line(data=nu, aes(x=game_date, y=team_fgpct), color="red") + 
  geom_line(data=mi, aes(x=game_date, y=team_fgpct), color="darkblue") +
