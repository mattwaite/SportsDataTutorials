...
correlations <- read_csv("https://www.mattwaite.com/sportsdatafiles/footballlogs25.csv")

residualmodel <- correlations |> 
  mutate(
    differential = team_pts - opponent_pts,
    net_yards = tot - opponent_tot
    )

fit <- lm(differential ~ net_yards, data = residualmodel)
summary(fit)

penalties <- correlations |> 
  mutate(
    differential = team_pts - opponent_pts, 
    total_penalties = penalties+opponent_penalties,
    total_penalty_yards = penalty_yds+opponent_penalty_yds
  )

pfit <- lm(differential ~ total_penalty_yards, data = penalties)
summary(pfit)

penresid <- penalties |> mutate(predicted = predict(pfit), residuals = residuals(pfit))

pensort <- penresid |> arrange(desc(residuals)) |> select(team, opponent, rslt, differential, total_penalty_yards, predicted, residuals) |> slice(1)
```
```{r correlations-load-data-exercise, exercise = TRUE}
correlations <- read_csv("https://www.mattwaite.com/sportsdatafiles/footballlogs24.csv")
```
...
Just like the headline says, it's time to create a linear model like we did in the last lesson. We'll create a linear model object called fit, and we'll use our residualmodel dataframe. In the model, we'll say that differential is approximately modeled by net_yards.
...
residualmodel |> 
  ...
  select(team, opponent, rslt, differential, net_yards, predicted, residuals)
```
...
ggplot(residualmodel, aes(x=net_yards, y=residuals)) + geom_point()
```
...
ggplot(data=residualmodel, aes(x=net_yards, y=differential)) + geom_point() + geom_segment(aes(xend = net_yards, yend = predicted)) + geom_smooth(method=lm, se=FALSE)
```
...
For this we'll make an object called pfit -- for penalty fit -- similar to what we did before. This time, we'll say differential is approximately modeled by total_penalty_yards. 
...
____ |> arrange(desc(____)) |> select(team, opponent, rslt, differential, total_penalty_yards, predicted, residuals)
```
...
penalties |> arrange(desc(residuals)) |> select(team, opponent, rslt, differential, total_penalty_yards, predicted, residuals)
```
```{r regression4-check}
grade_this_code()
```
...
ggplot(penalties, aes(x=total_penalty_yards, y=residuals)) + geom_point()
```
...
ggplot(data=penalties, aes(x=total_penalty_yards, y=differential)) + geom_point() + geom_segment(aes(xend = total_penalty_yards, yend = predicted)) + geom_smooth(method=lm, se=FALSE)
```
