...
dllink <- download_link(
  link = "http://mattwaite.github.io/sportsdatafiles/volleyball1821.csv",
...
seasons <- read_csv("http://mattwaite.github.io/sportsdatafiles/volleyball1821.csv")
...
To do this, we're going to do a few things we've done before -- we're going to mutate a new column calculating digs per set so we can compare season to season and then we'll use filter to get just Nebraska. We'll put that into a new dataframe called `huskers`.
...
ggplot() + 
  geom_bar(data=huskers, aes(x=Season, weight=digsperset))
```
```{r ggplot-bar2-check}
grade_this_code()
```

Now we get bars of different sizes, and you can clearly see a difference between 2020 and 2021 when Rodriguez joined the team. But does this tell a *clear* story? Debatable.
...
First, let's get a dataset of every Big Ten team in 2021 instead of just Nebraska. 
...
ggplot() + 
  geom_bar(data=big10, aes(x=reorder(Institution, digsperset), weight=digsperset)) 
```
```{r ggplot-bar4-check}
grade_this_code()
```

Better. And look who is the top team in digs per set. Surprise surprise. 
...
ggplot() + 
  geom_bar(data=big10, aes(x=reorder(Institution, ??????????), weight=??????????)) +
  ?????_????()
```
