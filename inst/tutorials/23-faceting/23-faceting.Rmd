...
We're going to use the logs of college basketball games last season. Per usual: **For purposes of this exercise, you don't need to do this. The data is included here if you want to try this in your own notebook.** 
...
We'll do something simple like a line chart of TeamFGPCT -- team field goal percentage -- and we're going to group on Team.
...
And, not surprisingly, we get a hairball. We could color certain lines, but that would limit us to focus on one team. **What if we did all of them at once?** We do that with a `facet_wrap`. 
...
ggplot() + 
  geom_hline(yintercept=.4446667, color="blue") + 
  geom_line(data=big10, aes(x=Date, y=TeamFGPCT, group=Team)) + 
  scale_y_continuous(limits = c(0, .7)) + 
  facet_wrap(~Team)
```
