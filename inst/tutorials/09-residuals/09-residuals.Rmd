...
Just like a previous exercise, we'll add precited and residuals to our dataframe. Then we'll arrange it, and use a little select magic to make it easier to read. 
...
????????? %>% arrange(desc(?????????)) %>% select(Team, Opponent, Result, differential, NetYards, predicted, residuals)
```
...
penalties %>% arrange(desc(residuals)) %>% select(Team, Opponent, Result, differential, NetYards, predicted, residuals)
```
```{r regression4-check}
grade_this_code()
```
