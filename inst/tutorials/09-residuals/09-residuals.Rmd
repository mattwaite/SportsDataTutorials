...
We've seen this output before, but let's review because if you are using scatterplots to make a point, you should do this. First, note the Min and Max residual at the top. A team has underperformed the model by nearly 36 points (!), and a team has overperformed it by 35 points (!!). The median residual, where half are above and half are below, is just slightly above the fit line. Close here is good. 

Next: Look at the Adjusted R-squared value. What that says is that 69 percent of a team's scoring output can be predicted by their net yards.  
...
* Our min and max go from -65 to positive 73
* Our adjusted R-squared is ... -0.0003596. Not much at all. 
* Our p-value is ... 0.5244, which is more than than .05. 
...
First, note all of the biggest misses here are all blowout games. The worst games of the 2021 season, the worst being Oklahoma vs Western Carolina. The model missed that differential by ... 73 points. The margin of victory? 76 points. In other words, this model is terrible. But let's look at it anyway. 
