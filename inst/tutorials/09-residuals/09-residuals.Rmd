...
Let's look at a net yards model from the last lesson. We need data from the 2022 college football season. **For purposes of this exercise, you don't need to do this. The data is included here if you want to try this in your own notebook.**
...
We've seen this output before, but let's review because if you are using scatterplots to make a point, you should do this. First, note the Min and Max residual at the top. A team has underperformed the model by nearly 44 points (!), and a team has overperformed it by 43 points (!!). The median residual, where half are above and half are below, is just slightly below the fit line. Close here is good. 

Next: Look at the Adjusted R-squared value. What that says is that 66 percent of a team's scoring output can be predicted by their net yards.  
...
* Our min and max go from -75 to positive 70
* Our adjusted R-squared is ... -0.0005768. Not much at all. 
* Our p-value is ... 0.8479, which is more than than .05. 
...
First, note all of the biggest misses here are all blowout games. The worst games of the 2022 season, the worst being Western Kentucky vs FIU. The model missed that differential by ... 73 points. The margin of victory? 73 points. In other words, this model is terrible. But let's look at it anyway. 
