...
Like usual, you already installed this if you followed the install instructions at the beginning of the course. But if you're having trouble getting this to work, go to your console in R Studio and install it with `devtools::install_github("mattwaite/ggalt")`
...
Let's say we want to highlight Brice Williams and his place in college basketball in the 2024-2025 season. Let's take a look at that season's player data. 
...
players <- read_csv("https://www.mattwaite.com/sportsdatafiles/players25.csv") |> filter(MP > 100)
...
players <- read_csv("https://www.mattwaite.com/sportsdatafiles/players25.csv") |>
  filter(MP > 100)
```
...
ggplot() + 
  geom_point(data=players, aes(x=PTS, y=PER))
```
```{r encircle1-check}
grade_this_code()
```
...
ggplot() + 
  ...
  annotate("rect", fill = "blue", alpha = 0.1, 
        ...
        ymin = ____, ymax = Inf) +
  geom_text(aes(x=300, y=35, label="Good player zone"), size=3) +
  labs(title="Brice Williams was Nebraska's MVP last season", subtitle="Not just Nebraska's best, but one of the best in the Big Ten.", x="Points scored", y="Player Efficiency Rating") + 
  theme_minimal() + 
  theme(
