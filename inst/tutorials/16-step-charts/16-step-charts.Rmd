...
nu <- bigdiff |> filter(Team == "Nebraska")
pu <- bigdiff |> filter(Team == "????????")
id <- bigdiff |> filter(Team == "????????")
```
...
ggplot() + 
  ...
  geom_step(data=??, aes(x=Date, y=CumDiff, group=Team), color="darkred") +
  annotate("text", x=(as.Date("2023-01-15")), y=-70, label="Nebraska") +
  annotate("text", x=(as.Date("2023-03-01")), y=250, label="Indiana") +
  annotate("text", x=(as.Date("2023-03-01")), y=350, label="Purdue") +
  labs(
