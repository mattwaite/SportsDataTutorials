...
Let's dive in using data on college softball in 2024. This workflow will represent a clear picture of what your work in this class will be like for much of the rest of the semester. One way to think of this workflow is that your R Notebook is now your digital sketchbook, where you will try different types of visualizations to find ones that work. Then, you will write the code that adds necessary and required parts to finish it. 
...
softball <- read_csv("https://www.mattwaite.com/sportsdatafiles/ncaa_softball_team_hitting_2025.csv")
...
Why `glimpse` here? The data because it's wide and has a lot of columns. 
...
Now, we should have 17 teams that we can look at. That's far more manageable than 309 -- the number of softball teams in Division I. 
...
`ggplot() + geom_bar(data=big10, aes(x=short_name))` 

We start with `ggplot()` which is creating a blank canvas for us to work in. The `geom_bar()` is the geometry -- the form the data will take. We will learn many geometries over the next several lessons. `big10` is our data, `aes` means aesthetics, `x=short_name` explicitly tells `ggplot2` that our x value -- our horizontal value -- is the short name of the team field from the data. Why short_name (or team if we wanted the long version)? We want one bar per team, no? Put the team on the x axis is what that is saying. And what do we get when we run that? 

```{r ggplot-bar1, exercise=TRUE, exercise.setup = "ggplot-load-data", message=FALSE}
ggplot() + 
  geom_bar(data=big10, aes(x=short_name))
```
...
ggplot() + 
  geom_bar(data=big10, aes(x=short_name, weight=____))
```
```{r ggplot-bar2-solution, exercise.reveal_solution = FALSE}
ggplot() + 
  geom_bar(data=big10, aes(x=short_name, weight=hr))
```
```{r ggplot-bar2-check}
grade_this_code()
```
...
ggplot() + 
  geom_bar(data=big10, aes(x=reorder(short_name, ____), weight=____))
```
```{r ggplot-bar3-solution, exercise.reveal_solution = FALSE}
ggplot() + 
  geom_bar(data=big10, aes(x=reorder(short_name, hr), weight=hr))
```
```{r ggplot-bar3-check}
grade_this_code()
```
...
First, we need to make a dataframe of one. Let's focus on Nebraska for ... reasons. Then, we just add another geom_bar to our chart.
...
ggplot() + 
  geom_bar(data=big10, aes(x=reorder(short_name, hr), weight=hr)) +
  geom_bar(data=huskers, aes(x=reorder(short_name, hr), weight=hr), fill="red")
```
```{r ggplot-bar4-check}
grade_this_code()
```
