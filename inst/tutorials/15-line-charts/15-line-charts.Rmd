...
ggplot() + 
  ...
  geom_line(data=??, aes(x=Date, y=TeamFGPCT), color="orange") +
  scale_y_continuous(limits = c(0, .7))
```
...
ggplot() + 
  ...
  geom_line(data=il, aes(x=Date, y=TeamFGPCT), color="orange") +
  scale_y_continuous(limits = c(0, .7))
```
```{r line3-check}
grade_this_code()
...
So visually speaking, the difference between Nebraska and Illinois' season is that Illinois mostly stayed evenly better than Nebraska and Nebraska was a bit more unpredictable from night to night.
...
Let's add all of the Big Ten to our line chart, but this time, we're going to fade them into the background by making them grey. Then, our red line for Nebraska and the green line of Michigan State will really stand out.
...
What do we see here? How has Nebraska and Illinois' season evolved against all the rest of the teams in college basketball?
