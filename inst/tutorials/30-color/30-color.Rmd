...
ggplot() + 
  geom_bar(data=big, aes(x=reorder(School, ??????????), weight=??????????)) + 
  coord_flip()
```
...
Let's make that base fade into the background by changing the color to light grey. With bar charts, we're changing the `fill`. Changing the `color` will only change the outline of the bar. 

```{r colors-bar2, exercise=TRUE, exercise.setup = "colors-load-data", message=FALSE}
ggplot() + 
  geom_bar(data=big, aes(x=reorder(School, ??????????), weight=??????????), fill="????? ????") + 
  coord_flip()
```
...
Now we're going to add in `dance` and `mi` to our chart. We want them to stand out from the rest of the B1G, so we're going to made `dance` dark grey and we'll use Michigan State's hex code to shade them in. 

```{r colors-bar3, exercise=TRUE, exercise.setup = "colors-load-data", message=FALSE}
ggplot() + 
  geom_bar(data=big, aes(x=reorder(School, OverallSRS), weight=OverallSRS), fill="????? ????") + 
  geom_bar(data=dance, aes(x=reorder(School, OverallSRS), weight=OverallSRS), fill="???? ????") +  
  geom_bar(data=??, aes(x=reorder(School, OverallSRS), weight=OverallSRS), fill="#18453B") +    
  coord_flip()
```
