---
title: "Sports Data Lesson 33: Rvest"
output: 
  learnr::tutorial:
    progressive: true
    allow_skip: true
...
The goal of this lesson is to introduce you to web scraping using the rvest package in R. By the end of this tutorial, you'll understand how to extract data from HTML tables on websites and import it into R for analysis. You'll learn how to identify the correct HTML elements to target, use XPath to select specific table data, and handle common challenges like unnamed columns and multi-row headers. We'll apply these techniques to real sports statistics websites, scraping college football and basketball data. You'll also practice cleaning and combining data from multiple years. This skill will allow you to automate data collection from web sources, saving time and enabling more up-to-date analysis of sports statistics that may not be available through official APIs or datasets.
...
Let's grab [a simple, basic HTML table from College Football Stats](https://cfbstats.com/2025/leader/national/team/offense/split01/category09/sort01.html). There's nothing particularly strange about this table -- it's simply formatted and easy to scrape. 
...
scoringoffenseurl <- "https://cfbstats.com/2025/leader/national/team/offense/split01/category09/sort01.html"
...
scoringoffenseurl24 <- "https://cfbstats.com/2024/leader/national/team/offense/split01/category09/sort01.html"
...
scoringoffense2024 <- scoringoffense24[[1]]

fixedscoringoffense2024 <- scoringoffense2024 |> rename(Rank = 1)|> mutate(YEAR = 2024)
fixedscoringoffense2025 <- scoring |> rename(Rank = 1) |> mutate(YEAR = 2025)

combined <- bind_rows(fixedscoringoffense2024, fixedscoringoffense2025)
...
fixedscoringoffense2024 <- scoringoffense2024 |> rename(Rank = 1) |> mutate(YEAR = ____)
fixedscoringoffense2025 <- scoring |> rename(Rank = 1) |> mutate(YEAR = ____)
```
...
combined <- bind_rows(____2024, ____2025)
```
...
Note if we run glimpse on the dataframe called combined that it has 257 rows -- which just so happens to be what the number of FBS teams added together totals for two seasons. 
...
Let's try to get [college basketball season team stats from Sports Reference](https://www.sports-reference.com/cbb/seasons/2025-school-stats.html). If you look at that page, you'll see the problem right away -- the headers span two rows, and they repeat (i.e. there's conference wins, home wins and away wins, but because the conference, home and away markers are on a different line, R won't know which W is which). That's going to be all kinds of no good. **You can't import that.** Dataframes must have names all in one row. If you have two-line headers, you have a problem you have to fix before you can do anything else with it.
...
In this lesson, we've explored the powerful capabilities of the rvest package for web scraping in R. We started with a simple example, extracting a basic HTML table from a college football statistics website. We then progressed to more complex scenarios, including combining data from multiple years and handling tables with non-standard headers. Throughout the tutorial, you've learned key rvest functions like read_html(), html_nodes(), and html_table(), as well as how to use XPath to target specific elements. We also covered important data cleaning steps, such as renaming columns, filtering out unwanted rows, and converting data types. By working through real-world examples with sports data, you've gained practical experience in overcoming common web scraping challenges. Remember, while web scraping can be a powerful tool for data collection, it's important to use it responsibly and in compliance with websites' terms of service. With these skills, you're now equipped to automate data collection from web sources, enabling more efficient and up-to-date sports analytics.

## Terms to Know

- `rvest`: An R package designed for web scraping, providing functions to easily extract data from HTML web pages.
- Web scraping: The process of automatically collecting data from websites using programming techniques.
- XPath: A query language used to navigate through elements and attributes in an XML or HTML document.
- `html_nodes()`: An rvest function used to select specific HTML elements from a web page based on CSS selectors or XPath.
- `html_table()`: An rvest function that extracts data from HTML tables and converts them into R data frames.
- `read_html()`: An rvest function that reads HTML content from a URL or file.
- `bind_rows()`: A dplyr function used to combine multiple data frames vertically, stacking them on top of each other.
- `mutate_at()`: A dplyr function that applies a transformation to multiple columns in a data frame.
- `rename()`: A dplyr function used to change the names of columns in a data frame.
- `header=FALSE`: An argument used in html_table() to indicate that the first row of the HTML table should not be treated as column names.